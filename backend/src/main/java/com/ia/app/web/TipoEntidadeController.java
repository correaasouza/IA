...
import com.ia.app.mapper.TipoEntidadeMapper;
...
public class TipoEntidadeController {
  ...
  public TipoEntidadeController(TipoEntidadeService service) {
  ...
  public ResponseEntity<Page<TipoEntidadeResponse>> list(
      ...
      Pageable pageable) {
    Instant maxUpdatedAt = service.maxUpdatedAt();
    ...
    Page<TipoEntidadeResponse> page = service.list(pageable).map(TipoEntidadeMapper::toResponse);
  ...
  public ResponseEntity<TipoEntidadeResponse> create(@Valid @RequestBody TipoEntidadeRequest request) {
    return ResponseEntity.ok(TipoEntidadeMapper.toResponse(service.create(request)));
  }

  @PutMapping("/{id}")
  @PreAuthorize("hasAnyRole('MASTER_ADMIN','TENANT_ADMIN')")
  public ResponseEntity<TipoEntidadeResponse> update(@PathVariable Long id,
      @Valid @RequestBody TipoEntidadeRequest request) {
    return ResponseEntity.ok(TipoEntidadeMapper.toResponse(service.update(id, request)));
  }
}
