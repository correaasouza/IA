...
@Service
public class ConfigColunaService {

  private final ConfigColunaRepository repository;

  public ConfigColunaService(ConfigColunaRepository repository) {
    this.repository = repository;
  }

  @Cacheable(value = "configColuna", key = "#tenantId + ':' + #screenId + ':' + #userId + ':' + #rolesKey")
  ...
  @CacheEvict(value = "configColuna", allEntries = true)
  public ConfigColuna save(ConfigRequest request, Long tenantId) {
    ConfigColuna entity = repository.findFirstByTenantIdAndScreenIdAndScopeTipoAndScopeValor(
      tenantId, request.screenId(), request.scopeTipo(), request.scopeValor()).orElse(new ConfigColuna());
